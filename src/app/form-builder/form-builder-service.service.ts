...
import { ButtonInterface } from '../shared/drag-drop-content/interfaces/button.interface';
import { CheckboxInterface } from '../shared/drag-drop-content/interfaces/checkbox.interface';
import { ElementInterface } from '../shared/drag-drop-content/interfaces/element.interface';
import { FormDesignInterface } from '../shared/drag-drop-content/interfaces/form-design.interface';
import { InputInterface } from '../shared/drag-drop-content/interfaces/input.interface';
import { MultipleSelectInterface } from '../shared/drag-drop-content/interfaces/multiple-select.interface';
import { PictureInterface } from '../shared/drag-drop-content/interfaces/picture.interface';
import { SeparatorInterface } from '../shared/drag-drop-content/interfaces/separator.interface';
import { SocialInterface } from '../shared/drag-drop-content/interfaces/social.interface';
import { SpacerInterface } from '../shared/drag-drop-content/interfaces/spacer.interface';
import { TextInterface } from '../shared/drag-drop-content/interfaces/text.interface';
import { UploaderInterface } from '../shared/drag-drop-content/interfaces/uploader.interface';
...
export class FormBuilderService {
  ...
  generateHtml(reference_id) {
    ...
    let url = this.apiUrl + 'file_forms/form/submit';
    let responseId = UUID.UUID();
    let formStyles = '';
    let pageStyles = '';
    ...
    formStyles += `border: solid ${this._formDesign['form_border_width']
        ? this._formDesign['form_border_width']
        : 0
      }px ${this._formDesign['form_border_color']
    ...
    html += `</head>
            <body class="d-flex min-vh-100" style="${pageStyles}">
              <form   [formGroup]="formSubmit" id="form" class="container-fluid user-select-auto mx-auto" style="${formStyles}">\n`;
    html += this.parseComponents(this._components);
    html += `\n   </form>  
              <div id="respImg" class="container-fluid user-select-auto mx-auto" style="${formStyles};display:none">
              <div style="text-align:center">
                <div style="border-radius:200px; height:200px; width:200px; background: #F8FAF5; margin:0 auto;">
              <i class="checkmark">✓</i>
            </div>
              <h1>Success</h1> 
              <p>We received your response;<br/> we'll be in touch shortly!</p>
            </div><div>          
            <script type="text/javascript" data-id="{{reference_id}}"> 
            // $("#form").validate();
            ...
            document.querySelector("#form").addEventListener('submit', function(e) {
              ...
              $(inputs).each(function () {
                 ...
                 element.push( {id: $(this).attr('id'),
                  value : $(this).prop("checked")
                 })
                } else if(($(this).attr('type')) === 'text')
                {element.push( {id: $(this).attr('id'),value : $(this).val()})
               }
               else if(($(this).attr('type')) === 'radio'){
                var radioId=element.find((num)=>  num.id==$(this).attr('name'))
               if($(this).prop("checked") ==true)
               { 
                element.push({id: $(this).attr('name'),value : $(this).val()})
              }
              else
               { 
                if(!radioId){
                   element.push({id: $(this).attr('name'),value : null})}
              }
    ...
    var inputData = {
      ...
      'refernece_id': "${reference_id}",
      'data': element
    }
    var formData = JSON.stringify(inputData);
    $.ajax({
      type: "POST",
      data: formData,
      url: "${url}",
      ...
      success: function (result, status, xhr) {
        // document.getElementById("#respImg").style.display = "show";
        // document.getElementById("#form").style.display = "none";
        $("#respImg").css("display", "block");
  ...
  parseButtonComponent(component: ElementInterface) {
    ...
    if (el.link_to) {
      ...
      //   ${el.content}
      // </button>`;
      $div += `<button type="submit" id="button-form" style="${buttonStyles}">
      ${el.content}
    </button>`;
    }
  ...
  parseInputComponent(component: ElementInterface) {
    ...
    switch (component['type']) {
      case 'date':
        html += `<div class="input-group date date-picker" id="date_input_${component.id
      ...
      case 'datetime':
        html += `<div class="input-group date datetime-picker" id="datetime_input_${component.id
          }" data-target-input="nearest">
                      <input type="text"
                          ...
                          style="${styles.join(';')}"
                          ${requiredAttr} />                    
                          <div class="input-group-text" data-toggle="datetimepicker" data-target="#datetime_input_${component.id
          }"><i class="fas fa-calendar-times"></i></div>                            
                      </div><div class="errorTxt"></div>`;
        break;

      case 'number':
        html += `<div><input type="number" class="${fieldClasses.join(' ')}"
        name="${el.field_name}"
        id="${component.id}"
        autocomplete="off"
        placeholder="${el.placeholder_text}"
            style="${styles.join(';')}" ${requiredAttr} /></div><div class="errorTxt"></div>`;
        break;

      default:
        html += `<div ><input type="${component.type}" class="${fieldClasses.join(
          ' '
        )}"
        name="${component.id}"
        id="${component.id}"
  ...
  parseRadiosComponent(component: ElementInterface) {
    ...
    for (let option of el.options) {
      options.push(`<div class="form-check">
                        <input class="form-check-input" type="radio" name="${component.id
        }" 
                          id="radio_${option.id}" value="${option.value}"${option.is_selected ? ' checked' : ''
        } ${requiredAttr} >
    ...
    html += `</label>${options.join('\n')} <div id="${component.id}" class="${component.id}"></div></div>`;
  ...
  parseDropdownComponent(component: ElementInterface) {
    ...
    for (let option of el.options) {
      options.push(
        `<option name="${component.id}" value="${option.value}"${option.is_selected ? ' selected' : ''
        }>${option.text}</option>`
      );
    }

    html += `</label>
        <select type="dropdown" class="${fieldClasses.join(' ')}"
            name="${el.field_name}"
            id="${component.id}"
            style="${styles.join(';')}"${requiredAttr}>
        ${options.join('\n')}
  ...
  parseCheckboxComponent(component: ElementInterface) {
    ...
    html += `<div class="form-check" style="${styles}">
        <input type="checkbox" class="form-check-input ms-0 me-2" name="${el.field_name
