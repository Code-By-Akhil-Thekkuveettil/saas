...
import socialIcons from '../shared/drag-drop-content/elements/social-property/social-icons';
...
export class FormBuilderComponent implements OnInit, OnDestroy {
  ...
  getFormHtml() {
    ...
    console.log(this.subscriptionForm.content_dnd.added_components, 'before')
   ...
   // publish and get anonymouslink
   publishForm(obj) {
    this.spinner.show();
  //  console.log(obj.added_components    ,'opp')
     obj.added_components.map((element: any, index) => { 
    element.index=index
  });
  // console.log(dbObj,obj.added_components)
    console.log(this.subscriptionForm.content_dnd.added_components, 'before',obj)
    this.docService.publishForm(obj,this.reference_id)
      .subscribe({
        next: (resp: any) => {
         this.getAnonymousLink(resp.data)
          this.spinner.hide();
        },
        error: (error: any) => {
          let err = '';
  ...
  onPreview() {
    ...
    modalRef.componentInstance.content = this.formBuilderService.generateHtml(this.reference_id);
  }
  ...
  //     2
  //   );
    this.publishForm(this.subscriptionForm.content_dnd)
    // console.log(obj)
  }
